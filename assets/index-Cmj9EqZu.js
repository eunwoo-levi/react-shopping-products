...
`;
const SkeletonBox$1 = newStyled.div`
  ...
  border-radius: 8px;
`;
const SkeletonWrapper$1 = newStyled.div`
  width: 100%;
  padding: 1rem;
  border-radius: 12px;
  ...
  gap: 0.75rem;
`;
const ImageSkeleton = newStyled(SkeletonBox$1)`
  width: 100%;
  height: 160px;
`;
const TextSkeleton = newStyled(SkeletonBox$1)`
  height: 20px;
  width: ${({ width }) => width || "100%"};
`;
const ButtonSkeleton = newStyled(SkeletonBox$1)`
  height: 36px;
  width: 100%;
`;
function ProductCardSkeleton() {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(SkeletonWrapper$1, { "data-testid": "product-skeleton", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(ImageSkeleton, {}),
    /* @__PURE__ */ jsxRuntimeExports.jsx(TextSkeleton, { width: "80%" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(TextSkeleton, { width: "50%" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(ButtonSkeleton, {})
  ] });
}
...
`;
const SkeletonBox = newStyled.div`
  background-color: #f0f0f0;
  border-radius: 8px;
  animation: ${pulse} 1.5s infinite ease-in-out;
`;
const NavbarSkeleton = newStyled.div`
  display: flex;
...
`;
const LogoSkeleton = newStyled(SkeletonBox)`
  width: 80px;
  height: 24px;
`;
const CartSkeleton = newStyled(SkeletonBox)`
  width: 32px;
  height: 32px;
  border-radius: 50%;
`;
...
function Skeleton() {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs(NavbarSkeleton, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(LogoSkeleton, {}),
      /* @__PURE__ */ jsxRuntimeExports.jsx(CartSkeleton, {})
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(SkeletonWrapper, { children: Array.from({ length: 20 }).map((_, index) => /* @__PURE__ */ jsxRuntimeExports.jsx(ProductCardSkeleton, {}, index)) })
  ] });
}
...
const getProducts = ({ sortValue }) => {
  return httpClient.get(`/products?sort=${sortValue}`);
};
const getCartProduct = ({ page = 0, size = 20 }) => {
  return httpClient.get(`/cart-items?page=${page}&size=${size}`);
};
...
const updateCartProduct = (cartProductId, cartProductQuantity) => {
...
const ProductsWithCartProvider = ({ children }) => {
  ...
  const fetchProducts = reactExports.useCallback(async () => {
    ...
    try {
      ...
      const cartProductIds = new Set(cartRes.content.map((cp) => cp.product.id));
      const mergedProducts = productRes.content.map((product) => {
        const cartProduct = cartRes.content.find((cp) => cp.product.id === product.id);
        return {
          ...product,
          isCart: cartProductIds.has(product.id),
          cartProductQuantity: (cartProduct == null ? void 0 : cartProduct.quantity) || 0,
          cartProductId: cartProduct == null ? void 0 : cartProduct.id
        };
      });
      setProducts(mergedProducts);
    } catch (error2) {
      if (error2 instanceof Error) {
        console.error("Error fetching products or cart:", error2);
  ...
  const toggleCartSelection = (productId) => {
    setSelectedProductIds((prev2) => {
      if (prev2.includes(productId)) {
        return prev2.filter((id2) => id2 !== productId);
      } else {
        return [...prev2, productId];
      }
    });
  };
  const updateCart = async (newCart) => {
    ...
    try {
      ...
      const findResult = cartRes.content.find((cp) => cp.product.id === newCart.productId);
      ...
      if (error2 instanceof Error) {
        console.error("Error updating cart:", error2);
  ...
  const removeFromCart = async (cartProductId, productId) => {
    setIsLoading(true);
    setError("");
    try {
      await deleteCartProduct(cartProductId);
      setSelectedProductIds((prev2) => prev2.filter((id2) => id2 !== productId));
      await fetchProducts();
    } catch (error2) {
      if (error2 instanceof Error) {
        console.error("Error removing from cart:", error2);
...
enableMocking().then(() => {
  client.createRoot(document.getElementById("root")).render(
    /* @__PURE__ */ jsxRuntimeExports.jsx(React.StrictMode, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(reactExports.Suspense, { fallback: /* @__PURE__ */ jsxRuntimeExports.jsx(Skeleton, {}), children: /* @__PURE__ */ jsxRuntimeExports.jsx(LazyApp, {}) }) })
  );
});
export {
